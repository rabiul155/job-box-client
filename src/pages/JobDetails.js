...
const JobDetails = () => {
  ...
  const [sendReplay, {}] = useReplyMutation();
  ...
  const submitQuestion = (data) => {
    sendQuestion({ ...data, jobId: _id, email: user.email, userId: user._id });
    reset();
  };

  const handleSendReplay = (id) => {
    const data = {
      reply,
      userId: id,
    };
  ...
  return (
    <div className="mx-6 pt-14 grid grid-cols-12 gap-5">
      <div className="col-span-9 mb-10">
        ...
        <div>
          <div>
            ...
            {user.role === "candidate" && (
              <form onSubmit={handleSubmit(submitQuestion)} action="">
                <div className="flex gap-3 my-5">
                  <input
